...
const App = {
  ...
  responseString: function() {
    var value = contract.methods.responseString().call();
    console.log(value);
  },
  responseInteger: function() {
    var value = contract.methods.responseInteger().call();
    console.log(value);
    value.then((vvv) => {$('#int').text(vvv)});
  },
  requestString: function() {
    var value = contract.methods.requestString('요청데이터').call();
    console.log(value);
  },
  requestInteger: function() {
    var value = contract.methods.requestInteger(1234).call();
    console.log(value);
  },
  addName: function(){
    ...
    var data = $('#data').val();
    contract.methods.addName(data).send({
      from:walletInstance.address,
      gas: '200000'
    })
  },
  getNames: function() {
    var names = contract.methods.getNames().call();
    console.log(names);
  },
  addUser: function() {
    var walletInstance = this.getWallet();
    var user_name = $('#user_name').val();
    contract.methods.addUser(user_name).send({
      from: walletInstance.address,
      gas: '200000'
    });
  },
  getUser: function() {
    var walletInstance = this.getWallet();
    var result = contract.methods.getUser(walletInstance.address).call();
    result.then(data => {
      console.log(data);
    });
  },
  getUserCount: function(){
    contract.methods.getUserCount().call().then(count => {
      $('#user_count').text(count+'명');
      $('#user_count').css('color','red');
    });
  }
};
window.App = App;
